...
public class QueueTest {
    ...
    void setUp() {
        client = new RedisBridgeClient() {
            ...
            public MessageRouter.Settings routerSettings() {
                return new MessageRouter.Settings(true, 50, 2, 5);
            }
        };
    ...
    void testPublishQueuedMessage() throws InterruptedException {
        ...
        TestMessage message = new TestMessage("queued test");
        client.getRedisRouter().publishQueued(message, client.platformEntity());

        assertTrue(latch.await(5, TimeUnit.SECONDS), "Packet should be received via queue");
    }
    ...
    void testPublishMultipleQueuedMessages() throws InterruptedException {
        ...
        for (int i = 0; i < messageCount; i++) {
            client.getRedisRouter().publishQueued(new TestMessage("queued test " + i), client.platformEntity());
        }
    ...
    void testQueuedMessageFutureCompletion() throws Exception {
        ...
        TestMessage message = new TestMessage("future test");
        CompletableFuture<Packet<TestMessage>> future = client.getRedisRouter()
                .publishQueued(message, client.platformEntity())
                .toCompletableFuture();

        Packet<TestMessage> sentMessage = future.get(5, TimeUnit.SECONDS);
    ...
    void testPublishQueuedMessageWithAck() throws Exception {
        ...
        client.getMessageRegistry()
                .register(AckEnabledMessage.class)
                .onReceive(fullMessage -> latch.countDown())
                .build();

        AckEnabledMessage message = new AckEnabledMessage("queued ack test");
        CompletableFuture<Packet<AckEnabledMessage>> future = client.getRedisRouter()
                .publishQueued(message, client.platformEntity())
                .toCompletableFuture();

        assertTrue(latch.await(5, TimeUnit.SECONDS), "Packet should be received via queue");
        Packet<AckEnabledMessage> sentMessage = future.get(5, TimeUnit.SECONDS);
        assertNotNull(sentMessage);
